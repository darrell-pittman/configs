...
#+begin_src emacs-lisp
  (defun wgc/tangle-config ()
    (when (let* ((buffer-check (apply-partially 'string= (buffer-file-name)))
                 (buffer-names '("emacs.org" "emacs-private.org"))
                 (expander (lambda (s) (f-expand s "~/configs/emacs-config/")))
                 (expanded-names (seq-map expander buffer-names)))
            (seq-some buffer-check expanded-names))
