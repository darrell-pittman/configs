  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (defun my/init-phms ()
      ...
      (let ((cur-buf (current-buffer)))
        ...
        (my/create-shell "clk" '("cd ~/p30m/phmsrails/phmsrails/lib/modules" "bundle exec clockwork clock.rb"))
    ...
    (defun my/kill-phms ()
      ...
      (interactive)
      (let* ((to-delete '("es" "rs" "clk" "csl"))
             (filtered (seq-filter (lambda (buf)
                                     (seq-contains to-delete (buffer-name buf)))
                                   (buffer-list))))
        (dolist (buf filtered)
          (kill-buffer buf))))
